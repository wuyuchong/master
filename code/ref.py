...
#  sns.pairplot(train_dataset[['固定资产比重(%)', '流动比率']])
...
pd.concat([train_labels.describe(), validation_labels.describe(), test_labels.describe(), backtest_labels.describe()], axis=1).to_csv('doc/outcome/train6/标签的分布.csv')
...
plt.savefig('doc/outcome/train6/学习率随迭代次数变化曲线.pdf')
...
def plot_history(history):
  ...
  plt.ylim([0,1])
  plt.legend()
  plt.savefig('doc/outcome/train6/随迭代次数增加损失的变化.pdf')
...
#  ).save('doc/outcome/train6/DNN在测试集上的表现.pdf')
...
plt.xlabel("Prediction Error")
plt.ylabel("Count")
plt.savefig('doc/outcome/train6/DNN在测试集和回测集上的残差分布.pdf')
...
table_combine = table_combine.query('prediction > -0.5 & prediction < 0.5 & actual > -0.5 & actual < 0.5')
(ggplot(table_combine, aes('actual', 'prediction'))
+ geom_point(alpha=0.1)
+ stat_smooth(method='glm')
+ facet_grid('. ~ dataset')
).save('doc/outcome/train6/DNN在测试集和回测集上的表现.pdf', height=4, width=10)
...
print(table)
table.to_csv('doc/outcome/train6/effect_predictions.csv', index=False)
...
print(table)
table.to_csv('doc/outcome/train6/predict_predictions.csv', index=False)
