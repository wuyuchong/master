...
```{r, out.width='25%', fig.align='center', fig.cap = "决策边界"}
knitr::include_graphics("figure/classification/Class Report.png")
```
...
```{r, out.width='25%', fig.align='center', fig.cap = "变量重要性"}
...
```{r, out.width='25%', fig.align='center', fig.cap = "验证曲线"}
...
```{r, out.width='25%', fig.align='center', fig.cap = "校准图"}
...
使用模型比较排名前三的 CatBoost、Light Gradiant Boosting、Extreme Gradient Boosting 模型分别进行 Boosting 和 Bagging 集成算法的搭建，发现在 Bagging 集成算法下三种模型都有一定的提升。
...
knitr::include_graphics("figure/classification/Residuals.png")
```
...
我们将测试集的数据也加入模型中进行训练，将训练后的模型应用于效应集上，观察模型的实际应用效果。
...
kable(table, caption = '各行业预测TOP公司组合与同行业所有公司营业收入比较')
```
